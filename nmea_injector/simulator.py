...
class Simulator(object):
    ...
    def start_auto_logging(self, filename=None):
        ...
        try:
            ...
            from datetime import datetime
            now = datetime.now()
            self._log_file_handle.write(f"# NMEA Log started at {now.strftime('%Y-%m-%d %H:%M:%S')}\n")
            self._log_file_handle.write(f"# Format: [timestamp] NMEA_sentence\n")
            self._log_file_handle.write(f"# Generated by NMEA Injector\n\n")
    ...
    def stop_auto_logging(self):
        ...
        if self._log_file_handle:
            try:
                ...
                from datetime import datetime
                now = datetime.now()
                self._log_file_handle.write(f"\n# NMEA Log ended at {now.strftime('%Y-%m-%d %H:%M:%S')}\n")
                self._log_file_handle.flush()
    ...
    def _add_to_stream(self, sentences):
        ...
        with self._stream_lock:
            for sentence in sentences:
                ...
                if self._log_file_handle:
                    try:
                        self._log_file_handle.write(f"[{timestamp}] {sentence}\n")
